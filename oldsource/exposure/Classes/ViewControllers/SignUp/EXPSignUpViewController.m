...
-(void)doneBtnPressed {
     NSDictionary *dict = @{@"user[first_name]":_firstNameField.text, @"user[last_name]": _lastNameField.text, @"user[email]":_emailField.text, @"user[username]":_usernameField.text, @"user[password]": _confirmPasswordField.text, @"user[phone]":_phoneField.text, @"user[device_token]":@""};
    [SVProgressHUD showWithStatus:@"Signing Up"];
    [User signUpUserWithAttributes:dict completion:^(User *user, NSError *error){
        if(!error){
          ...
          [SVProgressHUD showErrorWithStatus:[NSString stringWithFormat:@"Signup failed: %@", error.localizedDescription]];
        }
    }];
}
